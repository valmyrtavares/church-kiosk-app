...
const Offering = () => {
    ...
    // Passos: 1 = Valor, 2 = Identificação, 3 = Pagamento
    ...
    const handleIdentified = (data) => {
        setUserData(data);
        setStep(3); // Avança pra pagamento
    };
    ...
    const handlePayment = async () => {
        ...
        // Simular tempo de pagamento na maquininha
        setTimeout(async () => {
            try {
        ...
        }, 4000); // 4 segundos simulando a máquina
    };
    ...
    return (
        <div className={formStyles.container}>
            ...
            <h2 className={formStyles.title}>Contribuição de Oferta</h2>

            {step === 1 && (
                <form className={formStyles.card} onSubmit={(e) => { e.preventDefault(); if (amount) setStep(2); }}>
                    <div className={formStyles.inputGroup}>
                        <label>1. Valor da Oferta (R$)</label>
                        <input
                            type="number"
                            step="0.01"
                            value={amount}
            ...
            {step === 2 && (
                <>
                    ...
                    <div className={formStyles.actionRow} style={{ marginTop: '10px' }}>
                        <ActionButton type="button" variant="outline" fullWidth onClick={() => setStep(1)}>Voltar para Valor</ActionButton>
                    </div>
                </>
            )}
